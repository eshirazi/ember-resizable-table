...
export default Ember.Component.extend({
  ...
  buildCoordToCell() {
    ...
    function placeCell(x, y, cell) {
      if (x + cell.get('colSpan') > numColumns || y + cell.get('rowSpan') > numRows) {
        return false;
      }
  ...
  reLayout() {
    ...
    for (let colIndex = 0; colIndex < firstRowCells.length; colIndex++) {
      this.incrementProperty('numColumns', firstRowCells[colIndex].get('colSpan'));
    }
  ...
  resizeMouseMove(event) {
    ...
    let pair = this.getSize(resizeColumnOrRow, resizeIndex - 1) + this.getSize(resizeColumnOrRow, resizeIndex);
    let mouse;
